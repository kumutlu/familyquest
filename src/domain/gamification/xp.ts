...
export interface XpEventDocumentV1 {
  readonly id: string
...
function assertLogicalCompletionKey(value: string): void {
  ...
  for (const component of components.slice(1)) assertComponent(component, 'logicalCompletionKey component')
}

export function logicalCompletionKey(childId: string, taskId: string, periodKey: string): string {
...
export function taskXpEventId(key: string): string {
  assertLogicalCompletionKey(key)
  return `task_xp:${key}`
}

export function taskXpReversalEventId(key: string): string {
  assertLogicalCompletionKey(key)
  return `task_xp_reversal:${key}`
}

export function legacyBaselineEventId(familyId: string, childId: string): string {
...
function canonicalSnapshot(event: GamificationEventV1): string {
...
function assertEventDelta(event: GamificationEventV1): void {
 ...
 */
export function foldXpEvents(documents: readonly XpEventDocumentV1[]): number {
  const documentIds = new Set<string>()
  const snapshotsByIdempotencyKey = new Map<string, string>()
  let xpTotal = 0n

  for (const { id, event } of documents) {
    if (id.length === 0) throw new Error('XP event document ID must be non-empty')
    if (documentIds.has(id)) throw new Error(`Duplicate XP event document ID: ${id}`)
    documentIds.add(id)
  ...
  if (xpTotal < 0n) throw new Error('XP ledger must not be negative')
