...
describe('Wallet Permissions', () => {
  ...
  it('owner cannot create standalone wallet ledger without matching operation', async () => {
    const db = getDb('owner-1');
    await expect(setDoc(doc(db, 'families', 'fam-1', 'wallet_transactions', 'tx-1'), {
      ...
      parentRef: 'owner-1',
      createdAt: serverTimestamp()
    })).rejects.toThrow(); // Fails because wallet balance isn't updated in same batch
  });
...
fs.writeFileSync('tests/firestore/ownerPermissions.rules.test.ts', content);
