...
Occurrences: **3363** across **251** files.
...
| `bestStreak` | 240 |
| `level` | 177 |
| `xpProgressInLevel` | 124 |
| `xpToNextLevel` | 117 |
| `gamification_summaries` | 108 |
| `gamification_events` | 105 |
| `longestStreak` | 85 |
| `behaviour_events` | 84 |
| `gamification_state` | 43 |
| `weeklyPoints` | 43 |
| `redeemReward` | 42 |
| `task_occurrences` | 26 |
...
#### `firestore.indexes.json` — MIGRATE · Phase 1 · risk Low

> Indexes follow the new collections

| Line | Term | Operation | Source | Semantic meaning | Code |
|---|---|---|---|---|---|
| 14 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `"collectionGroup": "gamification_events",` |
...
| Line | Term | Operation | Source | Semantic meaning | Code |
|---|---|---|---|---|---|
| 44 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `* are derived here inside a single transaction. ˋusers.lifetimeXPˋ is written` |
| 45 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `* only as a COMPATIBILITY MIRROR of ˋgamification_summaries.xpTotalˋ and must` |
| 45 | `xpTotal` | read | families/{f}/gamification_summaries | Projection lifetime XP counter (authoritative today) | `* only as a COMPATIBILITY MIRROR of ˋgamification_summaries.xpTotalˋ and must` |
| 55 | `behaviour_events` | read | families/{f}/behaviour_events | Behaviour intent log | `const eventRef = familyRef.collection('behaviour_events').doc(args.behaviourEventId)` |
| 65 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summaryRef = familyRef.collection('gamification_summaries').doc(childId)` |
| 76 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `const gamificationEventRef = familyRef.collection('gamification_events').doc(behaviourGamificationEventId(args.behaviourEventId))` |
| 89 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `currentRewardPoints: integer(child.rewardPoints),` |
| 90 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `currentXpTotal: integer(summary?.xpTotal),` |
| 91 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `currentLifetimeXP: integer(child.lifetimeXP),` |
| 98 | `rewardPoints` | calculate | users.rewardPoints | Spendable Reward Points balance (RP) | `// which would silently discard the authoritative rewardPoints/summary` |
| 131 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints: plan.nextRewardPoints,` |
| 132 | `xpTotal` | calculate | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `// Compatibility-only mirror; authoritative XP is summary.xpTotal.` |
| 133 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `lifetimeXP: plan.nextLifetimeXP,` |
| 141 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `xpTotal: plan.nextXpTotal,` |
...
| Line | Term | Operation | Source | Semantic meaning | Code |
|---|---|---|---|---|---|
| 214 | `gamification_events` | test | families/{f}/gamification_events | Existing XP event ledger | `expect(store.paths().some((p) => p.includes('gamification_events'))).toBe(false)` |
| 271 | `gamification_events` | test | families/{f}/gamification_events | Existing XP event ledger | `expect(store.paths().some((p) => p.includes('gamification_events'))).toBe(false)` |
| 282 | `gamification_events` | test | families/{f}/gamification_events | Existing XP event ledger | `expect(store.paths().some((p) => p.includes('gamification_events'))).toBe(true)` |
| 315 | `gamification_events` | test | families/{f}/gamification_events | Existing XP event ledger | `expect(store.paths().filter((p) => p.includes('gamification_events')).length).toBe(1)` |
...
| Line | Term | Operation | Source | Semantic meaning | Code |
|---|---|---|---|---|---|
| 423 | `bestStreak` | initialise | summary.bestStreak | Projection best-streak counter | `schemaVersion: 1, familyId, childId, xpTotal: 0, level: 1, currentStreak: 0, bestStreak: 0,` |
| 423 | `currentStreak` | initialise | users.currentStreak | Consecutive qualifying days | `schemaVersion: 1, familyId, childId, xpTotal: 0, level: 1, currentStreak: 0, bestStreak: 0,` |
| 423 | `level` | initialise | in-memory / derived | Member level | `schemaVersion: 1, familyId, childId, xpTotal: 0, level: 1, currentStreak: 0, bestStreak: 0,` |
| 423 | `xpTotal` | initialise | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `schemaVersion: 1, familyId, childId, xpTotal: 0, level: 1, currentStreak: 0, bestStreak: 0,` |
| 456 | `xpTotal` | calculate | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `const nextXp = base.xpTotal + xpDelta` |
| 462 | `currentStreak` | read | users.currentStreak | Consecutive qualifying days | `let currentStreak = base.currentStreak` |
| 465 | `currentStreak` | read | users.currentStreak | Consecutive qualifying days | `currentStreak = 0` |
| 469 | `currentStreak` | read | users.currentStreak | Consecutive qualifying days | `currentStreak = base.lastQualifiedDayKey !== null && addFamilyDays(base.lastQualifiedDayKey, 1) === progress.dayKey` |
| 470 | `currentStreak` | calculate | users.currentStreak | Consecutive qualifying days | `? base.currentStreak + 1 : 1` |
| 477 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `xpTotal: nextXp,` |
| 478 | `level` | read | in-memory / derived | Member level | `level: levelForXp(nextXp, 1000),` |
| 479 | `currentStreak` | read | users.currentStreak | Consecutive qualifying days | `currentStreak,` |
| 480 | `bestStreak` | calculate | summary.bestStreak | Projection best-streak counter | `bestStreak: Math.max(base.bestStreak, currentStreak),` |
| 480 | `currentStreak` | calculate | users.currentStreak | Consecutive qualifying days | `bestStreak: Math.max(base.bestStreak, currentStreak),` |
| 603 | `task_occurrences` | read | families/{f}/task_occurrences | Server-side task occurrence dedupe records | `const occurrenceRef = familyRef.collection('task_occurrences').doc(logicalKey)` |
| 606 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summaryRef = familyRef.collection('gamification_summaries').doc(childId)` |
| 650 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `transaction.get(familyRef.collection('gamification_events').doc(taskXpEventId(logicalKey))),` |
| 651 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `transaction.get(familyRef.collection('gamification_events').doc(taskXpReversalEventId(logicalKey))),` |
| 653 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `.map(id => transaction.get(familyRef.collection('gamification_events').doc(id))),` |
| 685 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `const currentPoints = child.rewardPoints ?? 0` |
| 686 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `if (!Number.isSafeInteger(currentPoints) \|\| currentPoints < 0) throw new Error('Child rewardPoints is invalid')` |
| 688 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `if (!Number.isSafeInteger(nextPoints)) throw new Error('Child rewardPoints would exceed the safe integer range')` |
| 743 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `// Mirror the authoritative gamification_summaries.xpTotal into the` |
| 743 | `xpTotal` | read | families/{f}/gamification_summaries | Projection lifetime XP counter (authoritative today) | `// Mirror the authoritative gamification_summaries.xpTotal into the` |
| 744 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `// users.lifetimeXP compatibility field, in the SAME transaction, so the` |
| 746 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `const childUpdate: Record<string, unknown> = { lifetimeXP: summary.xpTotal }` |
| 746 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `const childUpdate: Record<string, unknown> = { lifetimeXP: summary.xpTotal }` |
| 748 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `childUpdate.rewardPoints = nextPoints` |
| 752 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `for (const document of plan.events) transaction.create(familyRef.collection('gamification_events').doc(document.id), eventToData(document.event))` |
| 792 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summaryRef = familyRef.collection('gamification_summaries').doc(effect.childId)` |
| 802 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `transaction.get(familyRef.collection('gamification_events').doc(taskXpEventId(effect.logicalCompletionKey))),` |
| 803 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `transaction.get(familyRef.collection('gamification_events').doc(taskXpReversalEventId(effect.logicalCompletionKey))),` |
| 805 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `.map(id => transaction.get(familyRef.collection('gamification_events').doc(id))),` |
| 836 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `const currentPoints = child.rewardPoints ?? 0` |
| 841 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `if (!Number.isSafeInteger(nextPoints) \|\| nextPoints < 0) throw new Error('Task invalidation would make rewardPoints invalid')` |
| 842 | `rewardPoints` | write | users.rewardPoints | Spendable Reward Points balance (RP) | `transaction.update(childRef, { rewardPoints: nextPoints })` |
| 844 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `for (const document of plan.events) transaction.create(familyRef.collection('gamification_events').doc(document.id), eventToData(document.event))` |
| 889 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summaryRef = familyRef.collection('gamification_summaries').doc(childId)` |
| 909 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `.map(id => transaction.get(familyRef.collection('gamification_events').doc(id))))` |
| 966 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `for (const document of events) transaction.create(familyRef.collection('gamification_events').doc(document.id), eventToData(document.event))` |
| 969 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `// Mirror the authoritative gamification_summaries.xpTotal into the` |
| 969 | `xpTotal` | read | families/{f}/gamification_summaries | Projection lifetime XP counter (authoritative today) | `// Mirror the authoritative gamification_summaries.xpTotal into the` |
| 970 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `// users.lifetimeXP compatibility field, in the SAME transaction, so the` |
| 972 | `lifetimeXP` | write | users.lifetimeXP | Legacy duplicate lifetime XP counter | `transaction.update(childRef, { lifetimeXP: summary.xpTotal })` |
| 972 | `xpTotal` | write | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `transaction.update(childRef, { lifetimeXP: summary.xpTotal })` |
| 988 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `const lifetimeXp = child.data().lifetimeXP` |
| 990 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `const baseline = await familyRef.collection('gamification_events').doc(ˋlegacy_xp_baseline:${encodeURIComponent(familyId)}:${encodeURIComponent(child.id)}ˋ).get` |
| 993 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summary = await familyRef.collection('gamification_summaries').doc(child.id).get()` |
| 1028 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `const eventQuery = this.rebuildQuery(familyRef.collection('gamification_events'), args.childId, checkpoint.watermarkAt, checkpoint.eventCursor)` |
| 1079 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summaryRef = familyRef.collection('gamification_summaries').doc(args.childId)` |
...
| Line | Term | Operation | Source | Semantic meaning | Code |
|---|---|---|---|---|---|
| 86 | `behaviour_events` | read | families/{f}/behaviour_events | Behaviour intent log | `'families/{familyId}/behaviour_events/{behaviourEventId}',` |
...
| 116 | `rewardPoints` | migrate | users.rewardPoints | Spendable Reward Points balance (RP) | `* (deltas = the replayed rewardPoints/xpTotal) and one rebuilt projection.` |
| 116 | `xpTotal` | migrate | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `* (deltas = the replayed rewardPoints/xpTotal) and one rebuilt projection.` |
| 148 | `rewardPoints` | migrate | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPointsDelta: replayed.rewardPoints,` |
| 149 | `xpTotal` | migrate | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `xpDelta: replayed.xpTotal,` |
...
| Line | Term | Operation | Source | Semantic meaning | Code |
|---|---|---|---|---|---|
| 122 | `rewardPoints` | initialise | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints: 0,` |
| 123 | `lifetimeXP` | initialise | users.lifetimeXP | Legacy duplicate lifetime XP counter | `lifetimeXP: 0,` |
| 124 | `currentStreak` | initialise | users.currentStreak | Consecutive qualifying days | `currentStreak: 0,` |
| 125 | `longestStreak` | initialise | users.longestStreak | Legacy best-streak counter on the user doc | `longestStreak: 0,` |
| 375 | `rewardPoints` | initialise | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints: 0,` |
| 376 | `lifetimeXP` | initialise | users.lifetimeXP | Legacy duplicate lifetime XP counter | `lifetimeXP: 0,` |
| 377 | `currentStreak` | initialise | users.currentStreak | Consecutive qualifying days | `currentStreak: 0,` |
| 378 | `longestStreak` | initialise | users.longestStreak | Legacy best-streak counter on the user doc | `longestStreak: 0,` |
| 452 | `rewardPoints` | initialise | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints: 0,` |
| 453 | `lifetimeXP` | initialise | users.lifetimeXP | Legacy duplicate lifetime XP counter | `lifetimeXP: 0,` |
| 454 | `currentStreak` | initialise | users.currentStreak | Consecutive qualifying days | `currentStreak: 0,` |
| 455 | `longestStreak` | initialise | users.longestStreak | Legacy best-streak counter on the user doc | `longestStreak: 0,` |
| 653 | `currentStreak` | read | users.currentStreak | Consecutive qualifying days | `let newCurrentStreak = userData.currentStreak \|\| 0;` |
| 654 | `longestStreak` | read | users.longestStreak | Legacy best-streak counter on the user doc | `let newLongestStreak = userData.longestStreak \|\| 0;` |
| 669 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `let finalRewardPoints = userData.rewardPoints \|\| 0;` |
| 670 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `let finalLifetimeXP = userData.lifetimeXP \|\| 0;` |
| 700 | `currentStreak` | read | users.currentStreak | Consecutive qualifying days | `currentStreak: newCurrentStreak,` |
| 701 | `longestStreak` | read | users.longestStreak | Legacy best-streak counter on the user doc | `longestStreak: newLongestStreak,` |
| 710 | `approveTaskCompletion` | read | in-memory / derived | V4 task completion command (RP + XP credit) | `export const approveTaskCompletion = async (familyId: string, completionId: string, comment?: string) => {` |
| 838 | `behaviour_events` | read | families/{f}/behaviour_events | Behaviour intent log | `const eventRef = doc(collection(db, ˋfamilies/${familyId}/behaviour_eventsˋ));` |
| 880 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints: child.rewardPoints ?? 0,` |
| 881 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `lifetimeXP: child.lifetimeXP ?? 0,` |
| 889 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `// writes gamification_summaries.xpTotal and the immutable gamification` |
| 889 | `xpTotal` | read | families/{f}/gamification_summaries | Projection lifetime XP counter (authoritative today) | `// writes gamification_summaries.xpTotal and the immutable gamification` |
| 970 | `createChallenge` | read | in-memory / derived | Family challenge configuration | `export const createChallenge = async (familyId: string, title: string, targetXP: number, rewardPoints: number, startXP: number) => {` |
| 970 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `export const createChallenge = async (familyId: string, title: string, targetXP: number, rewardPoints: number, startXP: number) => {` |
| 974 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints,` |
| 981 | `claimChallenge` | read | in-memory / derived | Family challenge claim (RP + XP credit) | `export const claimChallenge = async (familyId: string, challengeId: string, rewardPoints: number, childrenIds: string[], challengeTitle: string) => {` |
| 981 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `export const claimChallenge = async (familyId: string, challengeId: string, rewardPoints: number, childrenIds: string[], challengeTitle: string) => {` |
| 1001 | `rewardPoints` | calculate | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints: (userDoc.data().rewardPoints \|\| 0) + rewardPoints,` |
| 1002 | `lifetimeXP` | calculate | users.lifetimeXP | Legacy duplicate lifetime XP counter | `lifetimeXP: (userDoc.data().lifetimeXP \|\| 0) + rewardPoints` |
| 1002 | `rewardPoints` | calculate | users.rewardPoints | Spendable Reward Points balance (RP) | `lifetimeXP: (userDoc.data().lifetimeXP \|\| 0) + rewardPoints` |
| 1010 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `text: ˋFamily Challenge Completed: ${challengeTitle}! Everyone got +${rewardPoints} pts!ˋ,` |
| 1033 | `redeemReward` | read | in-memory / derived | Reward redemption (RP debit) | `export const redeemReward = async (` |
| 1060 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `const currentPoints = userDoc.data().rewardPoints \|\| 0;` |
| 1097 | `rewardPoints` | write | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints: currentPoints - cost,` |
| 1145 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `const currentPoints = userDoc.data().rewardPoints \|\| 0;` |
| 1146 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `const currentXP = userDoc.data().lifetimeXP \|\| 0;` |
| 1149 | `rewardPoints` | write | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints: currentPoints + points,` |
| 1150 | `lifetimeXP` | write | users.lifetimeXP | Legacy duplicate lifetime XP counter | `lifetimeXP: currentXP + points` |
| 3115 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `*  6. The exact point cost is deducted from ˋrewardPointsˋ.` |
| 3122 | `unlockAvatar` | read | in-memory / derived | Avatar unlock (RP debit) | `export const unlockAvatar = async (familyId: string, avatarId: string): Promise<number> => {` |
| 3147 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `const currentPoints = userData.rewardPoints \|\| 0;` |
| 3152 | `rewardPoints` | write | users.rewardPoints | Spendable Reward Points balance (RP) | `transaction.update(userRef, { rewardPoints: currentPoints - cost });` |
...
| Line | Term | Operation | Source | Semantic meaning | Code |
|---|---|---|---|---|---|
| 61 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `// pages the daily_eligibility and gamification_events streams for one child` |
| 76 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `collectionGroup: 'gamification_events',` |
| 292 | `behaviour_events` | read | families/{f}/behaviour_events | Behaviour intent log | `{ resource: 'behaviourEvents', key: 'behaviourEvents', kind: 'query', target: collection(db, ˋ${familyPath}/behaviour_eventsˋ) },` |
| 305 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `{ resource: 'gamificationSummaries', key: 'gamificationSummaries', kind: 'query', target: collection(db, ˋ${familyPath}/gamification_summariesˋ) },` |
| 313 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `{ resource: 'gamificationSummaries', key: 'gamificationSummaries', kind: 'document', target: doc(db, ˋ${familyPath}/gamification_summaries/${userId}ˋ) },` |
...
| 431 | `gamification_summaries` | test | families/{f}/gamification_summaries | Legacy projection collection | `// Parent can read: families/{familyId}/gamification_summaries` |
| 440 | `gamification_summaries` | test | families/{f}/gamification_summaries | Legacy projection collection | `// Child can read: families/{familyId}/gamification_summaries/{childId}` |
| 466 | `xpTotal` | test | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `expect(view.xpTotal).toBe(0);` |
| 480 | `xpTotal` | test | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `xpTotal: 1500,` |
| 481 | `level` | test | in-memory / derived | Member level | `level: 2,` |
| 482 | `xpToNextLevel` | test | summary.xpToNextLevel | XP remaining until the next level | `xpToNextLevel: 500,` |
| 483 | `xpProgressInLevel` | test | summary.xpProgressInLevel | XP accumulated inside the current level | `xpProgressInLevel: 500,` |
| 484 | `currentStreak` | test | users.currentStreak | Consecutive qualifying days | `currentStreak: 0,` |
| 485 | `bestStreak` | test | summary.bestStreak | Projection best-streak counter | `bestStreak: 0,` |
| 505 | `xpTotal` | test | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `xpTotal: 1500,` |
| 506 | `level` | test | in-memory / derived | Member level | `level: 2,` |
| 507 | `xpToNextLevel` | test | summary.xpToNextLevel | XP remaining until the next level | `xpToNextLevel: 500,` |
| 508 | `xpProgressInLevel` | test | summary.xpProgressInLevel | XP accumulated inside the current level | `xpProgressInLevel: 500,` |
| 509 | `currentStreak` | test | users.currentStreak | Consecutive qualifying days | `currentStreak: 0,` |
| 510 | `bestStreak` | test | summary.bestStreak | Projection best-streak counter | `bestStreak: 0,` |
| 529 | `xpTotal` | test | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `xpTotal: 1000,` |
| 530 | `level` | test | in-memory / derived | Member level | `level: 2,` |
| 531 | `xpToNextLevel` | test | summary.xpToNextLevel | XP remaining until the next level | `xpToNextLevel: 1000,` |
| 532 | `xpProgressInLevel` | test | summary.xpProgressInLevel | XP accumulated inside the current level | `xpProgressInLevel: 0,` |
| 533 | `currentStreak` | test | users.currentStreak | Consecutive qualifying days | `currentStreak: 1,` |
| 534 | `bestStreak` | test | summary.bestStreak | Projection best-streak counter | `bestStreak: 1,` |
