...
Occurrences: **3744** across **301** files.
...
| Line | Term | Operation | Source | Semantic meaning | Code |
|---|---|---|---|---|---|
| 53 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `* are derived here inside a single transaction. ˋusers.lifetimeXPˋ is written` |
| 54 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `* only as a COMPATIBILITY MIRROR of ˋgamification_summaries.xpTotalˋ and must` |
| 54 | `xpTotal` | read | families/{f}/gamification_summaries | Projection lifetime XP counter (authoritative today) | `* only as a COMPATIBILITY MIRROR of ˋgamification_summaries.xpTotalˋ and must` |
| 64 | `behaviour_events` | read | families/{f}/behaviour_events | Behaviour intent log | `const eventRef = familyRef.collection('behaviour_events').doc(args.behaviourEventId)` |
| 74 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summaryRef = familyRef.collection('gamification_summaries').doc(childId)` |
| 85 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `const gamificationEventRef = familyRef.collection('gamification_events').doc(behaviourGamificationEventId(args.behaviourEventId))` |
| 98 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `currentRewardPoints: integer(child.rewardPoints),` |
| 99 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `currentXpTotal: integer(summary?.xpTotal),` |
| 100 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `currentLifetimeXP: integer(child.lifetimeXP),` |
| 107 | `rewardPoints` | calculate | users.rewardPoints | Spendable Reward Points balance (RP) | `// which would silently discard the authoritative rewardPoints/summary` |
| 140 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints: plan.nextRewardPoints,` |
| 141 | `xpTotal` | calculate | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `// Compatibility-only mirror; authoritative XP is summary.xpTotal.` |
| 142 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `lifetimeXP: plan.nextLifetimeXP,` |
| 150 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `xpTotal: plan.nextXpTotal,` |
| 188 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `* for the delta math, the same ˋusers.rewardPointsˋ + ˋusers.lifetimeXPˋ` |
| 188 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `* for the delta math, the same ˋusers.rewardPointsˋ + ˋusers.lifetimeXPˋ` |
| 189 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `* compatibility mirror, the ˋgamification_summariesˋ projection, the` |
| 190 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `* immutable ˋgamification_eventsˋ ledger, and the V3 shadow. No parallel` |
| 207 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summaryRef = familyRef.collection('gamification_summaries').doc(args.childId)` |
| 219 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `const gamificationEventRef = familyRef.collection('gamification_events').doc(behaviourGamificationEventId(syntheticId))` |
| 232 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `currentRewardPoints: integer(child.rewardPoints),` |
| 233 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `currentXpTotal: integer(summary?.xpTotal),` |
| 234 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `currentLifetimeXP: integer(child.lifetimeXP),` |
| 271 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints: plan.nextRewardPoints,` |
| 272 | `xpTotal` | calculate | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `// Compatibility-only mirror; authoritative XP is summary.xpTotal.` |
| 273 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `lifetimeXP: plan.nextLifetimeXP,` |
| 280 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `xpTotal: plan.nextXpTotal,` |
...
| 337 | `lifetimeXP` | test | users.lifetimeXP | Legacy duplicate lifetime XP counter | `it('still denies client rewardPoints/lifetimeXP writes on users/{uid}', () => {` |
| 337 | `rewardPoints` | test | users.rewardPoints | Spendable Reward Points balance (RP) | `it('still denies client rewardPoints/lifetimeXP writes on users/{uid}', () => {` |
| 338 | `lifetimeXP` | test | users.lifetimeXP | Legacy duplicate lifetime XP counter | `expect(rules).toContain("'rewardPoints', 'lifetimeXP'")` |
| 338 | `rewardPoints` | test | users.rewardPoints | Spendable Reward Points balance (RP) | `expect(rules).toContain("'rewardPoints', 'lifetimeXP'")` |
