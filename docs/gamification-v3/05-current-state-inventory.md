...
Occurrences: **2043** across **162** files.
...
| `xpProgressInLevel` | 65 |
| `xpToNextLevel` | 65 |
| `behaviour_events` | 56 |
| `task_occurrences` | 23 |
| `weeklyPoints` | 23 |
| `levelProgressForXp` | 22 |
| `redeemReward` | 17 |
| `claimChallenge` | 11 |
| `unlockAvatar` | 9 |
| `levelFromXp` | 8 |
...
| 71 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `let rewardPoints = 0` |
| 72 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `let xpTotal = 0` |
| 73 | `weeklyPoints` | read | derived (client-computed today) | Weekly leaderboard score | `let weeklyPoints = 0` |
| 74 | `currentStreak` | read | users.currentStreak | Consecutive qualifying days | `let currentStreak = 0` |
| 75 | `bestStreak` | read | summary.bestStreak | Projection best-streak counter | `let bestStreak = 0` |
| 98 | `reversal` | read | families/{f}/reversals | Reversal / compensation path | `ˋreversalOfEventId ${event.reversalOfEventId} must be ordered before its reversalˋ,` |
| 105 | `reversal` | read | families/{f}/reversals | Reversal / compensation path | `// A reversal belongs to the weekly window of the event it corrects.` |
| 114 | `rewardPoints` | calculate | users.rewardPoints | Spendable Reward Points balance (RP) | `const nextReward = rewardPoints + event.rewardPointsDelta` |
| 119 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `ˋevent ${event.eventId} would drive rewardPoints negative (${nextReward}); reward points may never be negativeˋ,` |
| 122 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints = 0` |
| 124 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints = nextReward` |
| 127 | `xpTotal` | calculate | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `const nextXp = xpTotal + event.xpDelta` |
| 129 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `throw new ValidationErrorV3(ˋevent ${event.eventId} would drive xpTotal negative (${nextXp})ˋ)` |
| 131 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `xpTotal = nextXp` |
| 134 | `weeklyPoints` | calculate | derived (client-computed today) | Weekly leaderboard score | `weeklyPoints = Math.max(0, weeklyPoints + event.weeklyPointsDelta)` |
| 145 | `currentStreak` | read | users.currentStreak | Consecutive qualifying days | `currentStreak = 1` |
| 151 | `currentStreak` | calculate | users.currentStreak | Consecutive qualifying days | `currentStreak += 1` |
| 153 | `currentStreak` | read | users.currentStreak | Consecutive qualifying days | `currentStreak = 1` |
| 157 | `bestStreak` | calculate | summary.bestStreak | Projection best-streak counter | `bestStreak = Math.max(bestStreak, currentStreak)` |
| 157 | `currentStreak` | calculate | users.currentStreak | Consecutive qualifying days | `bestStreak = Math.max(bestStreak, currentStreak)` |
| 161 | `levelProgressForXp` | read | in-memory / derived | Canonical level formula | `const progress = levelProgressForXp(xpTotal, GAMIFICATION_CONFIG_V1.xpPerLevel)` |
| 161 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `const progress = levelProgressForXp(xpTotal, GAMIFICATION_CONFIG_V1.xpPerLevel)` |
| 166 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints,` |
| 167 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `xpTotal,` |
| 168 | `weeklyPoints` | read | derived (client-computed today) | Weekly leaderboard score | `weeklyPoints,` |
| 169 | `currentStreak` | read | users.currentStreak | Consecutive qualifying days | `currentStreak,` |
| 170 | `bestStreak` | read | summary.bestStreak | Projection best-streak counter | `bestStreak,` |
| 175 | `xpProgressInLevel` | read | summary.xpProgressInLevel | XP accumulated inside the current level | `xpProgressInLevel: progress.xpIntoLevel,` |
| 176 | `xpToNextLevel` | read | summary.xpToNextLevel | XP remaining until the next level | `xpToNextLevel: progress.xpToNextLevel,` |
