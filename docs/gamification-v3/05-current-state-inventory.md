...
Occurrences: **3264** across **240** files.
...
| `bestStreak` | 234 |
| `level` | 171 |
| `xpProgressInLevel` | 118 |
| `xpToNextLevel` | 111 |
| `gamification_summaries` | 108 |
| `gamification_events` | 94 |
| `longestStreak` | 85 |
| `behaviour_events` | 84 |
| `weeklyPoints` | 43 |
| `redeemReward` | 42 |
| `gamification_state` | 32 |
| `task_occurrences` | 26 |
...
| 95 | `rewardPoints` | calculate | users.rewardPoints | Spendable Reward Points balance (RP) | `// which would silently discard the authoritative rewardPoints/summary` |
| 128 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `rewardPoints: plan.nextRewardPoints,` |
| 129 | `xpTotal` | calculate | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `// Compatibility-only mirror; authoritative XP is summary.xpTotal.` |
| 130 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `lifetimeXP: plan.nextLifetimeXP,` |
| 138 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `xpTotal: plan.nextXpTotal,` |
...
| 742 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `// Mirror the authoritative gamification_summaries.xpTotal into the` |
| 742 | `xpTotal` | read | families/{f}/gamification_summaries | Projection lifetime XP counter (authoritative today) | `// Mirror the authoritative gamification_summaries.xpTotal into the` |
| 743 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `// users.lifetimeXP compatibility field, in the SAME transaction, so the` |
| 745 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `const childUpdate: Record<string, unknown> = { lifetimeXP: summary.xpTotal }` |
| 745 | `xpTotal` | read | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `const childUpdate: Record<string, unknown> = { lifetimeXP: summary.xpTotal }` |
| 747 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `childUpdate.rewardPoints = nextPoints` |
| 751 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `for (const document of plan.events) transaction.create(familyRef.collection('gamification_events').doc(document.id), eventToData(document.event))` |
| 791 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summaryRef = familyRef.collection('gamification_summaries').doc(effect.childId)` |
| 801 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `transaction.get(familyRef.collection('gamification_events').doc(taskXpEventId(effect.logicalCompletionKey))),` |
| 802 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `transaction.get(familyRef.collection('gamification_events').doc(taskXpReversalEventId(effect.logicalCompletionKey))),` |
| 804 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `.map(id => transaction.get(familyRef.collection('gamification_events').doc(id))),` |
| 835 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `const currentPoints = child.rewardPoints ?? 0` |
| 840 | `rewardPoints` | read | users.rewardPoints | Spendable Reward Points balance (RP) | `if (!Number.isSafeInteger(nextPoints) \|\| nextPoints < 0) throw new Error('Task invalidation would make rewardPoints invalid')` |
| 841 | `rewardPoints` | write | users.rewardPoints | Spendable Reward Points balance (RP) | `transaction.update(childRef, { rewardPoints: nextPoints })` |
| 843 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `for (const document of plan.events) transaction.create(familyRef.collection('gamification_events').doc(document.id), eventToData(document.event))` |
| 886 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summaryRef = familyRef.collection('gamification_summaries').doc(childId)` |
| 906 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `.map(id => transaction.get(familyRef.collection('gamification_events').doc(id))))` |
| 963 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `for (const document of events) transaction.create(familyRef.collection('gamification_events').doc(document.id), eventToData(document.event))` |
| 966 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `// Mirror the authoritative gamification_summaries.xpTotal into the` |
| 966 | `xpTotal` | read | families/{f}/gamification_summaries | Projection lifetime XP counter (authoritative today) | `// Mirror the authoritative gamification_summaries.xpTotal into the` |
| 967 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `// users.lifetimeXP compatibility field, in the SAME transaction, so the` |
| 969 | `lifetimeXP` | write | users.lifetimeXP | Legacy duplicate lifetime XP counter | `transaction.update(childRef, { lifetimeXP: summary.xpTotal })` |
| 969 | `xpTotal` | write | summary.xpTotal | Projection lifetime XP counter (authoritative today) | `transaction.update(childRef, { lifetimeXP: summary.xpTotal })` |
| 985 | `lifetimeXP` | read | users.lifetimeXP | Legacy duplicate lifetime XP counter | `const lifetimeXp = child.data().lifetimeXP` |
| 987 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `const baseline = await familyRef.collection('gamification_events').doc(ˋlegacy_xp_baseline:${encodeURIComponent(familyId)}:${encodeURIComponent(child.id)}ˋ).get` |
| 990 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summary = await familyRef.collection('gamification_summaries').doc(child.id).get()` |
| 1025 | `gamification_events` | read | families/{f}/gamification_events | Existing XP event ledger | `const eventQuery = this.rebuildQuery(familyRef.collection('gamification_events'), args.childId, checkpoint.watermarkAt, checkpoint.eventCursor)` |
| 1076 | `gamification_summaries` | read | families/{f}/gamification_summaries | Legacy projection collection | `const summaryRef = familyRef.collection('gamification_summaries').doc(args.childId)` |
