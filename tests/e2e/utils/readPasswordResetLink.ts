...
if (!email) throw new Error('RESET_EMAIL_REQUIRED');
...
const match = [...(body.oobCodes ?? [])].reverse().find(item =>
  item.email === email && item.requestType === 'PASSWORD_RESET' && item.oobLink,
);
if (!match?.oobLink) throw new Error('RESET_LINK_NOT_FOUND');
