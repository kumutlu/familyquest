...
if (!rules.includes(oldFn)) {
  console.error('ERROR: oldFn not found in firestore.rules. Aborting.');
  process.exit(1);
}

rules = rules.replace(oldFn, newFn);
fs.writeFileSync(rulesPath, rules, 'utf8');
console.log('OK: isValidMoneyRequestApproval optimized (removed redundant get of requester wallet).');
